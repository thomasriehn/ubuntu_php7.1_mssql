...
RUN sudo pecl install sqlsrv pdo_sqlsrv
#RUN sudo echo "extension= pdo_sqlsrv.so" >> `php --ini | grep "Loaded Configuration" | sed -e "s|.*:\s*||"`
#RUN sudo echo "extension= sqlsrv.so" >> `php --ini | grep "Loaded Configuration" | sed -e "s|.*:\s*||"`
RUN sudo echo  "extension= pdo_sqlsrv.so" >> /etc/php/7.1/apache2/php.ini
RUN sudo echo "extension= sqlsrv.so" >> /etc/php/7.1/apache2/php.ini
# Enable apache mods.
RUN a2enmod php7.1
RUN a2enmod rewrite
